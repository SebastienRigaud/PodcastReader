...
function RssFeed()
{
	...
	this.fieldsUpdateFromXML = function(xmlInput)
	{
		this.title=rssContent.getElementsByTagName("title")[0].textContent;
		this.linkTag=rssContent.getElementsByTagName("link")[0].textContent;
		this.description=rssContent.getElementsByTagName("description")[0].textContent;
	}
	...
	this.extractRssFrom = function(fileUrl)
	{
		...
		req.onreadystatechange = function() {
			if(req.readyState === 4)
			{
				...
				{
					console.log(this);
					fieldsUpdateFromXML(req.responseXML);
				}
				else
				{
					console.log("failed");
